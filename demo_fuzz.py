...
def _parse_spans(spans: list[dict]) -> list[TraceNode]:
    ...
    for span in spans:
        nodes.append(TraceNode(
            ...
            content=f"{span.get('class_name', '').split('.')[-1]}.{span.get('method_name', '')}",
            function=span.get("method_name", ""),
            method_signature=span.get("method_signature", ""),
            class_namespace=span.get("class_name", ""),
            src_file=span.get("src_file", ""),
            line_number=span.get("line_number", -1),
            start_ns=span.get("start_epoch_ns", 0),
