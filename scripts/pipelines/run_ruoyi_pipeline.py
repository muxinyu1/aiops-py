#!/usr/bin/env python3
"""
run_ruoyi_pipeline.py — RuoYi-Cloud ruoyi-auth 全自动化 Fuzz Pipeline
...
用法:
  set -a && source .env && set +a && .venv/bin/python run_ruoyi_pipeline.py
"""
...
RUOYI_ROOT = str(PROJECT_ROOT / "examples" / "RuoYi-Cloud")
# ruoyi-auth 是入口服务源码; GlobalExceptionHandler 在 ruoyi-common-security (依赖 jar, 容器内可达)
RUOYI_SOURCES = [
    str(PROJECT_ROOT / "examples" / "RuoYi-Cloud" / "ruoyi-auth"),
    str(PROJECT_ROOT / "examples" / "RuoYi-Cloud" / "ruoyi-common" / "ruoyi-common-security"),
]

PACKAGE_FILTER = "com.ruoyi"
PROJECT_NAME = "ruoyi"
...
# 缓存路径
JOERN_CG_CACHE = str(PROJECT_ROOT / "ruoyi-callgraph.json")
CODEQL_DB_PATH = "/tmp/codeql-db-ruoyi"
CODEQL_CSV_PATH = "/tmp/ruoyi-taint-results.csv"

# Fuzz 配置
BASE_URL = "http://localhost:8080"
CONTAINER_NAME = "trace-real-ruoyi-cloud-ruoyi-auth"
ATTACK_MARKER = "FUZZ_MARKER_7x9k"
SINKS_JSON = str(PROJECT_ROOT / "sinks" / "ruoyi-logging-sinks.json")

# Sink 白名单: ruoyi-auth 容器内可达的 GlobalExceptionHandler sink
# (GlobalExceptionHandler 在 ruoyi-common-security 依赖 jar 中, 随 auth 容器加载, 框架反射调用可达)
# 注入点: requestURI 用户可控 (URL 路径带 marker), 触发各类异常即入日志
# 已实测: handleException L123 (兜底) 对不存在 URL 触发 NoResourceFoundException, requestURI 带 marker ✅
# 格式: "{className}.{methodName}"
SINK_WHITELIST = {
    # handleException/handleRuntimeException 是兜底型, 需请求不匹配任何 Controller 的 URL,
    # 但这种路径 trace 无法覆盖 (无 X-Execution-Trace 头), 不适合 fuzz
    "com.ruoyi.common.security.handler.GlobalExceptionHandler.handleHttpRequestMethodNotSupported",
    # handleHttpRequestMethodNotSupported: 对已注册路由(如 /login)用错误 HTTP 方法触发,
    # 请求到 DispatcherServlet 匹配阶段, trace 有效, requestURI 含 marker
}
...
def _add_exception_handler_edges(cg: CallGraph) -> int:
    """
    添加 Spring @RestControllerAdvice 全局异常处理合成边.

    GlobalExceptionHandler 由 Spring MVC 在 Controller 抛异常时反射调用,
    Joern 静态分析看不到该调用边. 手动为 TokenController 各入口方法添加
    到 GlobalExceptionHandler 各处理方法的边, 使 PathGenerator 能找到路径.

    注: 异常处理是"横切"逻辑, 理论上任何 Controller 异常都可能路由到
    GlobalExceptionHandler. 这里连 TokenController 的入口方法作为代表.
    """
    CONTROLLERS = [
        "com.ruoyi.auth.controller.TokenController.login",
        "com.ruoyi.auth.controller.TokenController.refresh",
        "com.ruoyi.auth.controller.TokenController.register",
    ]
    HANDLERS = [
        "com.ruoyi.common.security.handler.GlobalExceptionHandler.handleException",
        "com.ruoyi.common.security.handler.GlobalExceptionHandler.handleRuntimeException",
        "com.ruoyi.common.security.handler.GlobalExceptionHandler.handleHttpRequestMethodNotSupported",
        "com.ruoyi.common.security.handler.GlobalExceptionHandler.handleNotPermissionException",
        "com.ruoyi.common.security.handler.GlobalExceptionHandler.handleNotRoleException",
    ]
    ...
    for ctl in CONTROLLERS:
        if ctl not in cg.nodes:
            logger.warning(f"  合成边: 未找到入口节点 {ctl}")
            continue
        for h in HANDLERS:
            if h not in cg.nodes:
                logger.warning(f"  合成边: 未找到 handler 节点 {h}")
                continue
            if not _has_edge(ctl, h):
                cg.add_edge(CallGraphEdge(caller_id=ctl, callee_id=h, is_taint=True))
                added += 1
...
def step1_joern_call_graph(force: bool = False) -> CallGraph:
    """使用 Joern 生成 RuoYi-Cloud ruoyi-auth 调用图."""
    logger.info("=" * 60)
    ...
    # 扫 RuoYi-Cloud 根 (含 ruoyi-auth + ruoyi-common-security, 包过滤 com.ruoyi)
    # GlobalExceptionHandler 在 common-security, 必须包含进来
    scan_root = RUOYI_ROOT
...
def step2_codeql_taint(cg: CallGraph, force: bool = False) -> CallGraph:
    ...
    logger.info(f"  项目根: {RUOYI_ROOT}")
    logger.info(f"  数据库: {CODEQL_DB_PATH}")

    try:
        result = adapter.mark_taint_edges(
            cg,
            source_root=RUOYI_ROOT,
            package_filter=PACKAGE_FILTER,
            database_dir=CODEQL_DB_PATH,
            build_command="mvn compile -DskipTests -q -pl ruoyi-auth -am",
        )
        logger.info(f"  Taint 对: {result.total_taint_pairs}, 标记边: {result.marked_edges}")
        logger.info(f"  结果已缓存")
...
def step3_generate_paths(cg: CallGraph, all_sinks: bool = False) -> list[tuple[APIEntry, Sink, ExpectedPath]]:
    ...
    # API 入口在 ruoyi-auth 的 Controller
    api_entries = discover_api_entries(RUOYI_SOURCES[0])
    ...
    if all_sinks:
        # 只保留 GlobalExceptionHandler 的 sink (common-security 依赖 jar, 容器内可达)
        reachable_sinks = [s for s in sinks if "GlobalExceptionHandler" in s.class_name]
        logger.info(f"  Sink 总数: {len(sinks)}, 可达 (GlobalExceptionHandler): {len(reachable_sinks)}")
    else:
        reachable_sinks = [
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="RuoYi-Cloud ruoyi-auth 全自动 Fuzz Pipeline")
