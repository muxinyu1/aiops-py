...
# Sink 白名单: 只 fuzz 这 3 个 (对应 demo 的 3 个目标, 不上全部 10 个)
# 格式: "{className}.{methodName}"
SINK_WHITELIST = {
    "io.github.xxyopen.novel.book.service.impl.BookServiceImpl.getBookContentAbout",
    "io.github.xxyopen.novel.common.auth.JwtUtils.parseToken",
    "io.github.xxyopen.novel.config.exception.CommonExceptionHandler.handlerException",
}
...
def _add_exception_handler_edges(cg: CallGraph) -> int:
    """
    添加 Spring 全局异常处理器合成边.

    CommonExceptionHandler 上的 @ExceptionHandler 方法由 Spring 框架在
    Controller 抛出异常时调用, 静态调用图看不到这条边.
    本函数手动添加:
      FrontBookController.getBookContentAbout → CommonExceptionHandler.<各 @ExceptionHandler 方法>
    使 PathGenerator 能找到从 API 入口到达异常处理器 sink 的路径
    (trace 已证实 content API 请求会经 CommonExceptionHandler 处理器).
    """
    HANDLER_PREFIX = "io.github.xxyopen.novel.config.exception.CommonExceptionHandler."
    HANDLER_METHODS = [
        "handlerException",
        "handlerBindException",
        "handlerBusinessException",
    ]
    CONTENT_CONTROLLER = "io.github.xxyopen.novel.book.controller.front.FrontBookController.getBookContentAbout"

    added = 0
    if CONTENT_CONTROLLER not in cg.nodes:
        return added

    existing_callees = [e.callee_id for e in cg.edges if e.caller_id == CONTENT_CONTROLLER]
    for method in HANDLER_METHODS:
        handler_node = HANDLER_PREFIX + method
        if handler_node not in cg.nodes:
            continue
        if handler_node not in existing_callees:
            edge = CallGraphEdge(caller_id=CONTENT_CONTROLLER, callee_id=handler_node, is_taint=True)
            cg.add_edge(edge)
            added += 1
...
def step1_joern_call_graph(force: bool = False) -> CallGraph:
    ...
    if os.path.exists(JOERN_CG_CACHE) and not force:
        ...
        # 添加异常处理器合成边
        added_handler = _add_exception_handler_edges(cg)
        logger.info(f"  异常处理器边: +{added_handler}")
        return cg
...
def step3_generate_paths(cg: CallGraph, all_sinks: bool = False) -> list[tuple[APIEntry, Sink, ExpectedPath]]:
    ...
    if all_sinks:
        # 全部 sink (排除 novel-search 等不在本容器的)
        reachable_sinks = [s for s in sinks if "search" not in s.class_name.lower()]
        logger.info(f"  Sink 总数: {len(sinks)}, 可达 (排除 search): {len(reachable_sinks)}")
    else:
        # 只保留白名单中的 3 个 sink (对应 demo 目标)
        reachable_sinks = [
...
def _select_best_targets(
    ...
    # 按 (API, sink) 分组: 每个 sink 各保留一条最佳路径, 使每个 sink 都成为独立 fuzz 目标
...
if __name__ == "__main__":
    ...
    parser.add_argument("--all-sinks", action="store_true", help="跑全部 sink (不限白名单 3 个)")
