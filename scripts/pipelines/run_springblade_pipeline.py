#!/usr/bin/env python3
"""
run_springblade_pipeline.py — SpringBlade blade-auth 全自动化 Fuzz Pipeline
...
用法:
  set -a && source .env && set +a && .venv/bin/python run_springblade_pipeline.py
"""
...
SPRINGBLADE_ROOT = str(PROJECT_ROOT / "examples" / "SpringBlade")
# blade-auth 是目标服务的源码 (sink 在 granter, 入口在 controller, 都在 blade-auth)
SPRINGBLADE_SOURCES = [
    str(PROJECT_ROOT / "examples" / "SpringBlade" / "blade-auth"),
]

PACKAGE_FILTER = "org.springblade"
...
# 缓存路径
JOERN_CG_CACHE = str(PROJECT_ROOT / "springblade-callgraph.json")
CODEQL_DB_PATH = "/tmp/codeql-db-springblade"
CODEQL_CSV_PATH = "/tmp/springblade-taint-results.csv"

# Fuzz 配置
BASE_URL = "http://localhost:8080"
CONTAINER_NAME = "trace-real-springblade-blade-auth"
ATTACK_MARKER = "FUZZ_MARKER_7x9k"
SINKS_JSON = str(PROJECT_ROOT / "sinks" / "springblade-logging-sinks.json")

# Sink 白名单: 只 fuzz blade-auth 内可达的 2 个 granter sink
# (AuthFilter.unAuth 在 blade-gateway, 不在本容器, 不可达)
# 格式: "{className}.{methodName}"
SINK_WHITELIST = {
    "org.springblade.auth.granter.CaptchaTokenGranter.grant",
    "org.springblade.auth.granter.PasswordTokenGranter.grant",
}
...
def _add_granter_dispatch_edges(cg: CallGraph) -> int:
    """
    添加 SpringBlade TokenGranter 运行时分派合成边.

    AuthController.token 通过 TokenGranterBuilder.getGranter(grantType) 从
    ConcurrentHashMap Bean 池取出 ITokenGranter, 再调用 granter.grant().
    这是运行时字符串分派, Joern 静态分析只能看到对接口 ITokenGranter.grant 的调用
    (或根本看不到), 无法解析到具体的 PasswordTokenGranter/CaptchaTokenGranter.

    本函数手动添加:
      AuthController.token → ITokenGranter.grant (若缺失)
      ITokenGranter.grant  → PasswordTokenGranter.grant
      ITokenGranter.grant  → CaptchaTokenGranter.grant
      AuthController.token → PasswordTokenGranter.grant  (直连兜底)
      AuthController.token → CaptchaTokenGranter.grant   (直连兜底)
    使 PathGenerator 能找到从 API 入口到达 granter sink 的路径.
    """
    CONTROLLER = "org.springblade.auth.controller.AuthController.token"
    IFACE = "org.springblade.auth.granter.ITokenGranter.grant"
    GRANTERS = [
        "org.springblade.auth.granter.PasswordTokenGranter.grant",
        "org.springblade.auth.granter.CaptchaTokenGranter.grant",
    ]
    ...
    # Controller → 接口
    if IFACE in cg.nodes and not _has_edge(CONTROLLER, IFACE):
        cg.add_edge(CallGraphEdge(caller_id=CONTROLLER, callee_id=IFACE, is_taint=True))
        added += 1

    for granter in GRANTERS:
        if granter not in cg.nodes:
            logger.warning(f"  合成边: 未找到 granter 节点 {granter}")
            continue
        # 接口 → 实现
        if IFACE in cg.nodes and not _has_edge(IFACE, granter):
            cg.add_edge(CallGraphEdge(caller_id=IFACE, callee_id=granter, is_taint=True))
            added += 1
        # Controller → 实现 (直连兜底, 保证连通)
        if not _has_edge(CONTROLLER, granter):
            cg.add_edge(CallGraphEdge(caller_id=CONTROLLER, callee_id=granter, is_taint=True))
            added += 1
...
def step1_joern_call_graph(force: bool = False) -> CallGraph:
    """使用 Joern 生成 SpringBlade blade-auth 调用图."""
    logger.info("=" * 60)
    ...
    if os.path.exists(JOERN_CG_CACHE) and not force:
        ...
        added_gr = _add_granter_dispatch_edges(cg)
        logger.info(f"  Granter 分派边: +{added_gr}")
        return cg
    ...
    # 只扫 blade-auth 模块 (sink + 入口都在此)
    scan_root = SPRINGBLADE_SOURCES[0]
...
def step2_codeql_taint(cg: CallGraph, force: bool = False) -> CallGraph:
    ...
    logger.info(f"  项目根: {SPRINGBLADE_ROOT}")
    logger.info(f"  数据库: {CODEQL_DB_PATH}")

    try:
        result = adapter.mark_taint_edges(
            cg,
            source_root=SPRINGBLADE_ROOT,
            package_filter=PACKAGE_FILTER,
            database_dir=CODEQL_DB_PATH,
            build_command="mvn compile -DskipTests -q -pl blade-auth -am",
        )
        logger.info(f"  Taint 对: {result.total_taint_pairs}, 标记边: {result.marked_edges}")
        logger.info(f"  结果已缓存")
...
def step3_generate_paths(cg: CallGraph, all_sinks: bool = False) -> list[tuple[APIEntry, Sink, ExpectedPath]]:
    ...
    api_entries = discover_api_entries(SPRINGBLADE_SOURCES[0])
    # blade-auth 单模块, 保留全部入口
    ...
    if all_sinks:
        # 排除 gateway (不在本容器)
        reachable_sinks = [s for s in sinks if "gateway" not in s.class_name.lower()]
        logger.info(f"  Sink 总数: {len(sinks)}, 可达 (排除 gateway): {len(reachable_sinks)}")
    else:
        reachable_sinks = [
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="SpringBlade blade-auth 全自动 Fuzz Pipeline")
