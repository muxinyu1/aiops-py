#!/usr/bin/env python3
"""
run_yudao_pipeline.py — yudao-cloud system-server 全自动化 Fuzz Pipeline
...
用法:
  set -a && source .env && set +a && .venv/bin/python run_yudao_pipeline.py
"""
...
YUDAO_ROOT = str(PROJECT_ROOT / "examples" / "yudao-cloud")
# system-server 是业务服务源码 (sink + 入口都在此)
YUDAO_SOURCES = [
    str(PROJECT_ROOT / "examples" / "yudao-cloud" / "yudao-module-system" / "yudao-module-system-server"),
]

PACKAGE_FILTER = "cn.iocoder.yudao"
PROJECT_NAME = "yudao"
...
# 缓存路径
JOERN_CG_CACHE = str(PROJECT_ROOT / "yudao-callgraph.json")
CODEQL_DB_PATH = "/tmp/codeql-db-yudao"
CODEQL_CSV_PATH = "/tmp/yudao-taint-results.csv"

# Fuzz 配置
BASE_URL = "http://localhost:8080"
CONTAINER_NAME = "trace-real-yudao-cloud-yudao-module-system-server"
ATTACK_MARKER = "FUZZ_MARKER_7x9k"
SINKS_JSON = str(PROJECT_ROOT / "sinks" / "yudao-logging-sinks.json")

# Sink 白名单: system-server 内需"业务失败分支 + 业务参数注入"的难 sink
# 黑盒 fuzz 成本极高: marker 需作为业务参数流入, 且要触发特定业务失败分支 (非显见 requestURI)
# 格式: "{className}.{methodName}"
SINK_WHITELIST = {
    # 获取用户/部门/岗位为空时打印查询 value —— 需让解析函数拿到不存在的标识
    "cn.iocoder.yudao.module.system.framework.operatelog.core.AdminUserParseFunction.apply",
    "cn.iocoder.yudao.module.system.framework.operatelog.core.DeptParseFunction.apply",
    "cn.iocoder.yudao.module.system.framework.operatelog.core.PostParseFunction.apply",
}
...
def _add_exception_handler_edges(cg: CallGraph) -> int:
    """yudao 无需框架异常处理合成边 (通用虚分派已覆盖 Controller→ServiceImpl)."""
    return 0
...
def step1_joern_call_graph(force: bool = False) -> CallGraph:
    """使用 Joern 生成 yudao system-server 调用图."""
    logger.info("=" * 60)
    ...
    # 只扫 system-server 模块 (sink + 入口都在此, 避免全量扫描过慢)
    scan_root = YUDAO_SOURCES[0]
...
def step2_codeql_taint(cg: CallGraph, force: bool = False) -> CallGraph:
    ...
    logger.info(f"  项目根: {YUDAO_ROOT}")
    logger.info(f"  数据库: {CODEQL_DB_PATH}")

    try:
        result = adapter.mark_taint_edges(
            cg,
            source_root=YUDAO_ROOT,
            package_filter=PACKAGE_FILTER,
            database_dir=CODEQL_DB_PATH,
            build_command="mvn compile -DskipTests -q -pl yudao-module-system/yudao-module-system-server -am",
        )
        logger.info(f"  Taint 对: {result.total_taint_pairs}, 标记边: {result.marked_edges}")
        logger.info(f"  结果已缓存")
...
def step3_generate_paths(cg: CallGraph, all_sinks: bool = False) -> list[tuple[APIEntry, Sink, ExpectedPath]]:
    ...
    # API 入口在 system-server 的 Controller
    api_entries = discover_api_entries(YUDAO_SOURCES[0])
    ...
    if all_sinks:
        # 只保留 system-server 模块的 sink
        reachable_sinks = [s for s in sinks if "system" in s.class_name.lower()]
        logger.info(f"  Sink 总数: {len(sinks)}, 可达 (system): {len(reachable_sinks)}")
    else:
        reachable_sinks = [
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="yudao system-server 全自动 Fuzz Pipeline")
