#!/usr/bin/env python3
"""
run_javams_pipeline.py — java-microservice 参数组合靶场 Fuzz Pipeline
...
用法:
  set -a && source .env && set +a && .venv/bin/python run_javams_pipeline.py
"""
...
JAVAMS_ROOT = str(PROJECT_ROOT / "examples" / "java-microservice")
JAVAMS_SOURCES = [
    str(PROJECT_ROOT / "examples" / "java-microservice"),
]
...
# 缓存路径
JOERN_CG_CACHE = str(PROJECT_ROOT / "javams-callgraph.json")
CODEQL_DB_PATH = "/tmp/codeql-db-javams"
CODEQL_CSV_PATH = "/tmp/javams-taint-results.csv"

# Fuzz 配置
BASE_URL = "http://localhost:8080"
CONTAINER_NAME = "trace-real-java-microservice"
ATTACK_MARKER = "FUZZ_MARKER_7x9k"
SINKS_JSON = str(PROJECT_ROOT / "sinks" / "javams-logging-sinks.json")

# Sink 白名单: 参数组合靶场 sink
# OrderServiceImpl.evaluateRisk: log.error 在三层 if 嵌套 (level=vip 且 amount>10000 且 channel=app*)
# 黑盒即使知道字段名, 不看源码也难以猜出组合 —— 必须偏差反馈逐层逼近
# 格式: "{className}.{methodName}"
SINK_WHITELIST = {
    "com.example.microservice.service.OrderService.evaluateRisk",
}
...
def step1_joern_call_graph(force: bool = False) -> CallGraph:
    """使用 Joern 生成 java-microservice 调用图."""
    logger.info("=" * 60)
    ...
    # 扫 java-microservice 模块
    scan_root = JAVAMS_SOURCES[0]
...
def step3_generate_paths(cg: CallGraph, all_sinks: bool = False) -> list[tuple[APIEntry, Sink, ExpectedPath]]:
    ...
    # API 入口在 java-microservice 的 Controller
    api_entries = discover_api_entries(JAVAMS_SOURCES[0])
    ...
    if all_sinks:
        # 只保留 java-microservice 的 sink
        reachable_sinks = [s for s in sinks if "microservice" in s.class_name]
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="java-microservice 参数组合靶场 Fuzz Pipeline")
