#!/usr/bin/env python3
"""
run_youlai_oms_pipeline.py — youlai-mall mall-oms 全自动化 Fuzz Pipeline
...
用法:
  set -a && source .env && set +a && .venv/bin/python run_youlai_oms_pipeline.py
"""
...
YOULAI_ROOT = str(PROJECT_ROOT / "examples" / "youlai-mall")
# mall-oms/oms-boot 是业务服务源码 (sink + 入口都在此)
YOULAI_SOURCES = [
    str(PROJECT_ROOT / "examples" / "youlai-mall" / "mall-oms" / "oms-boot"),
]

PACKAGE_FILTER = "com.youlai"
PROJECT_NAME = "youlai-oms"
...
# 缓存路径
JOERN_CG_CACHE = str(PROJECT_ROOT / "youlai-oms-callgraph.json")
CODEQL_DB_PATH = "/tmp/codeql-db-youlai-oms"
CODEQL_CSV_PATH = "/tmp/youlai-oms-taint-results.csv"

# Fuzz 配置
BASE_URL = "http://localhost:8803"
CONTAINER_NAME = "trace-real-youlai-mall-oms"
ATTACK_MARKER = "FUZZ_MARKER_7x9k"
SINKS_JSON = str(PROJECT_ROOT / "sinks" / "youlai-logging-sinks.json")

# Sink 白名单: mall-oms 内"业务参数注入 + 需偏差反馈"的难 sink
# submitOrder L174: log.info("订单提交参数:{}", JSONUtil.toJsonStr(submitForm))
#   - marker 藏在请求 body 多层嵌套表单 (orderToken/orderItems/shippingAddress)
#   - 需过 @Validated (5 必填字段+枚举+嵌套对象), 黑盒极难一次构造对
#   - 已手工验证: marker 放 orderToken 可注入 ✅
# 格式: "{className}.{methodName}"
SINK_WHITELIST = {
    "com.youlai.mall.oms.service.app.impl.OrderServiceImpl.submitOrder",
    # confirmOrder 的 sink 参数是 memberId(来自JWT)/skuId(Long), 字符串 marker 不可注入, 已实测
    # "com.youlai.mall.oms.service.app.impl.OrderServiceImpl.confirmOrder",
}
...
def _add_exception_handler_edges(cg: CallGraph) -> int:
    """youlai-oms 无需框架异常处理合成边 (通用虚分派已覆盖 Controller→ServiceImpl)."""
    return 0
...
def step1_joern_call_graph(force: bool = False) -> CallGraph:
    """使用 Joern 生成 youlai mall-oms 调用图."""
    logger.info("=" * 60)
    ...
    # 只扫 mall-oms/oms-boot 模块 (sink + 入口都在此, 避免全量扫描过慢)
    scan_root = YOULAI_SOURCES[0]
...
def step2_codeql_taint(cg: CallGraph, force: bool = False) -> CallGraph:
    ...
    logger.info(f"  项目根: {YOULAI_ROOT}")
    logger.info(f"  数据库: {CODEQL_DB_PATH}")

    try:
        result = adapter.mark_taint_edges(
            cg,
            source_root=YOULAI_ROOT,
            package_filter=PACKAGE_FILTER,
            database_dir=CODEQL_DB_PATH,
            build_command="mvn compile -DskipTests -q -pl mall-oms/oms-boot -am",
        )
        logger.info(f"  Taint 对: {result.total_taint_pairs}, 标记边: {result.marked_edges}")
        logger.info(f"  结果已缓存")
...
def step3_generate_paths(cg: CallGraph, all_sinks: bool = False) -> list[tuple[APIEntry, Sink, ExpectedPath]]:
    ...
    # API 入口在 mall-oms 的 Controller
    api_entries = discover_api_entries(YOULAI_SOURCES[0])
    ...
    if all_sinks:
        # 只保留 mall-oms 模块的 sink
        reachable_sinks = [s for s in sinks if ".mall.oms." in s.class_name]
        logger.info(f"  Sink 总数: {len(sinks)}, 可达 (mall-oms): {len(reachable_sinks)}")
    else:
        reachable_sinks = [
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="youlai mall-oms 全自动 Fuzz Pipeline")
